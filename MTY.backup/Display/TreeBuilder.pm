...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($split_leading_spaces_from_text_re
     ...
     dependency_graph_to_tree_recursive get_shaded_tree_label
     indented_text_to_tree labels_and_levels_to_tree
     split_text_into_arrays_of_lines_and_indents
     tree_of_hashes_to_printable_tree);
...
noexport:; sub dependency_graph_to_tree_recursive {
  ...
  my $dep_set_hash_key = (!$dep_count) ? 0 :
    pack('L['.$dep_count.']', (map { # should really be Q (64-bit) but L is good enough for hashing
      my $p = $key_to_deplist->{$_}; 
      (refaddr($p) >> 3);
    } @$deps));

  my $same_deps_as_key = ($dep_count > 0) ? $repeated_direct_dep_sets->{$dep_set_hash_key} : undef;
  ...
  if ((defined $same_deps_as_key) && ($already_visited_all_direct_deps) && 
    ...
    my $identical_dep_set_node = [ [
      ...
      R_3_4.double_left_paren.R.'same set of '.ORANGE.
        $dep_count.R.' deps as '.ORANGE,
