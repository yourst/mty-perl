...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(CR EB GB KB MB NL TB ESC TAB bit max min DUAL bits bool enum null UNDEF
     IO_REF SCALAR STRING addrof clipto is_ref restof sizeof typeof BILLION
     MILLION REF_REF bitmask classof cond_or extenum flatten inrange is_dual
     join_if lowbits refaddr set_max set_min CODE_REF DUAL_REF GLOB_REF
     HASH_REF THOUSAND TRILLION all_true any_true enumbits if_there is_empty
     is_there multikey prealloc ARRAY_REF CLASS_REF RATIO_1_2 RATIO_1_3
     RATIO_1_4 RATIO_2_3 RATIO_3_4 UNDEF_REF all_empty all_false all_there
     any_empty any_false any_there getfields hash_args is_in_set is_scalar
     is_string none_true pad_array set_ifdef setfields undefs_to ALL_8_BITS
     FORMAT_REF LVALUE_REF OBJECT_REF RATIO_1_10 RATIO_9_10 REGEXP_REF
     SCALAR_REF STRING_REF array_args count_true first_true flattenref
     is_numeric is_ref_ref is_tainted is_vstring named_args none_false
     set_clipto %empty_hash ALL_16_BITS ALL_32_BITS ALL_48_BITS ALL_64_BITS
     BLESSED_REF QUADRILLION UNKNOWN_REF VSTRING_REF all_defined any_defined
     count_empty count_false count_there first_there is_code_ref is_glob_ref
     is_hash_ref max_in_list min_in_list ref_to_args remove_dups $zero_scalar
     @empty_array bit_names_of filter_pairs is_array_ref is_class_ref
     is_read_only set_if_empty set_if_there sparse_array $empty_scalar
     $undef_scalar all_undefined any_undefined count_defined first_defined
     is_object_ref is_ref_typeid is_regexp_ref is_scalar_ref list_contains
     multikey_list remove_undefs blessed_typeof generic_typeid generic_typeof
     is_blessed_ref slice_of_array $empty_hash_ref count_undefined
     first_specified push_if_defined shifted_bitmask $empty_array_ref
     is_array_or_hash is_scalar_typeid join_with_sep_if ratio_to_percent
     unblessed_typeof $empty_scalar_ref @typeid_to_string first_true_result
     undefs_to_inplace mutually_exclusive ref_to_first_undef
     first_defined_result is_scalar_ref_typeid pair_list_with_value
     remove_dups_in_place MODIFY_REF_ATTRIBUTES index_list_to_bitmask
     named_or_ordered_args MODIFY_CODE_ATTRIBUTES MODIFY_HASH_ATTRIBUTES
     first_non_empty_result remove_undefs_in_place MODIFY_ARRAY_ATTRIBUTES
     ...
     cond_and_index_list_to_bitmask copy_array_elements_where_undef
     undefs_to_empty_strings_inplace __sort_by_length_then_alphabetical
     sort_by_ascending_length_then_alphabetical split_dual
     sort_by_descending_length_then_alphabetical reflist is_glob
     arrays_identical hashes_identical
     escape_metachars escape_metachars_if_unescaped);

# For convenience, re-export these functions from 
# Data::Alias, List::Util and List::MoreUtils:
push @EXPORT, qw(alias copy reduce any all none notall first 
...
# The multikey function can be intuitively invoked as follows:
#
# my %hash = (
#   ... => ...,
...
#
# restof(@array or $arrayref, $offset) returns any elements after the
...
sub restof(+;$) {
  my ($array, $offset) = @_;
  $offset //= 1;
  return ( ref_to_args @{$array}[$offset..$#{$array}] );
}
