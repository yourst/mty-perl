...
sub get_file_stats($;$$$) {
  ...
  $relative_path = strip_trailing_slash($relative_path);
...
sub flush_file_stats_cache {
  ...
  printdebug{'Invalidating file stats cache for paths: ', @_};

  if (@_) {
    foreach my $path (@_) { $file_stats_cache->invalidate($path); }
  } else {
    $file_stats_cache->flush(); # just flush everything
  }
}
...
sub invalidate_cached_stats_for_path { 
  foreach my $path (@_) {
    flush_file_stats_cache($path) if (is_string $path); 
  } 
}

sub invalidate_cached_stats_for_fd {
  my $fd = get_native_fd($_[0]);
  printdebug{'Invalidating file stats cache for fd ', $fd};
  my $path = path_of_open_fd($fd);
  flush_file_stats_cache($path) if (defined $path);
}

INIT {
  push @invalidate_cached_path_hooks, \&invalidate_cached_stats_for_path;
  push @invalidate_cached_fd_hooks, \&invalidate_cached_stats_for_fd;
}
