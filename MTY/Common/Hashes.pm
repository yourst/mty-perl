...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(slice keysof print_hash invert_hash merge_hashes hash_to_scope
     slice_as_hash array_to_hash_keys array_values_to_hash
     print_hash_as_string hash_contains_all_keys hash_to_array_of_pairs
     pair_of_arrays_to_hash array_to_histogram_hash append_to_hash_of_arrays
     count_keys_matching_list hash_is_missing_all_keys
     hash_contains_any_of_keys add_missing_arrays_to_hash
     add_missing_hashes_to_hash add_missing_scalars_to_hash
     invert_hash_dups_to_subarrays hash_sorted_by_keys_as_pair_array
     key_list_of_hash_sorted_by_values value_list_of_hash_sorted_by_keys
     merge_hashes_and_collect_conflicts merge_hashes_without_empty_strings
     append_to_hash_of_arrays_or_scalars hash_sorted_by_values_as_pair_array
     symbol_name_array_to_hash_with_values
     remove_leading_trailing_space_on_hash_and_remove_if_empty);

sub pair_of_arrays_to_hash(++;+) {
  my ($keys, $values, $hash) = @_;

  if (defined $hash) {
    # pre-allocate buckets for what we're adding:
    keys %$hash = (scalar keys %$hash) + (scalar @$keys);
    @{$hash}{$keys} = @$values;
  } else {
    return (wantarray
              ? zip(@$keys, @$values) 
...
sub array_values_to_hash(+) {
  ...
  return (wantarray ? 
            ( map { $_ => $value_array_ref->[$_] } keys @$value_array_ref ) :
            { map { $_ => $value_array_ref->[$_] } keys @$value_array_ref });
}

sub array_to_hash_keys(+;$+) {
  my ($keys, $value, $hash) = @_;
  ...
  if (defined $hash) {
    # pre-allocate buckets for what we're adding:
    keys %$hash = (scalar keys %$hash) + (scalar @$keys);
    @{$hash}{$keys} = ($value) x (scalar @$keys);
    return ((defined wantarray) ? (wantarray ? %$hash : $hash) : ( ));
  } else {
    return (wantarray
              ? ( map { $_ => $value } @$keys )
              : { map { $_ => $value } @$keys });
...
sub hash_to_array_of_pairs(+) {
  my ($hash) = @_;
  return (wantarray ? ( List::Utils::pairs %$hash ) : [ List::Utils::pairs %$hash ]);
}
...
sub merge_hashes(++;$$) {
  my ($target, $source, $overwrite_if_conflict) = @_;
  $overwrite_if_conflict //= 1;

  my $target_key_count = scalar keys %$target;
  my $source_key_count = scalar keys %$source;
  # pre-allocate hash buckets for source keys:
  keys %$target = ($target_key_count + $source_key_count);

  if ($overwrite_if_conflict) {
    @{$target}{keys %$source} = values %$source;
  } else {
    while (my ($k, $v) = each %$source) {
      if (exists $target->{$k}) { push @$conflicts, $k; } else { $target->{$k} = $v };
    }
  }
  my $new_keys_added = (scalar keys %$target) - $target_key_count;
  my $collision_count = $source_key_count - $new_keys_added;
  return (wantarray ? ($collision_count, $new_keys_added) : $collision_count);
}
...
sub merge_hashes_and_collect_conflicts(++;+$) {
  ...
  while (my ($k, $v) = each %$source) { 
    ...
    push @$conflicts, $k if ($e);
    $target->{$k} = $v if ((!$e) || $overwrite_if_conflict);
  }
...
sub key_list_of_hash_sorted_by_values(+;$) {
  my ($h, $numeric_sort) = @_;
  $string_sort //= grep { (!looks_like_number $_) } values %$h;
  return ($string_sort 
