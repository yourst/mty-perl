...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(add_missing_arrays_to_hash add_missing_hashes_to_hash
     ...
     append_to_hash_of_arrays_or_scalars array_pair_to_hash
     array_to_hash_keys array_to_histogram_hash count_keys_matching_list
     hash_contains_all_keys hash_contains_any_of_keys
     hash_is_missing_all_keys hash_to_array_of_pairs hash_to_scope
     invert_hash invert_hash_dups_to_subarrays
     key_list_of_hash_sorted_by_values merge_hashes
     merge_hashes_and_collect_conflicts merge_hashes_without_empty_strings
     print_hash print_hash_as_string
     remove_leading_trailing_space_on_hash_and_remove_if_empty slice_of_hash
     symbol_name_array_to_hash_with_values value_list_of_hash_sorted_by_keys);
...
sub print_hash(+;$) {
  ...
  print($fd print_hash_as_string($h));
}
...
sub key_list_of_hash_sorted_by_values(+) {
  my $h = $_[0];
  return sort { ord($h->{$b}) <=> ord($h->{$a}) } keys %{$h};
}
...
# 
#   my %hash = aliased_keys_hash abc => 123, ['def', 'ghi'] => 456, ...
#
...
sub slice_of_hash(+;@) {
  ...
  my $newhash = { };
  my $first = $_[0];
  my $keylist = (is_scalar $first) ? \@_ :
                (is_array_ref $first) ? $first :
                (is_hash_ref $first) ? [ keys %$first ] : undef;
                   
  foreach my $key (@$keylist) { $newhash->{$key} = $hash->{$key}; }
  return $newhash;
}
