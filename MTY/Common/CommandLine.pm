...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($command_line_option_re $key_value_option_re
     ...
     OPTION_HASH_MANIPULATION OPTION_HELP_CATEGORY OPTION_HELP_DEFAULT
     OPTION_HELP_LITERAL OPTION_HELP_SYNTAX OPTION_HELP_WRAPPED OPTION_INTEGER
     ...
     add_styles_to_description check_and_expand_prioritized_file_lists
     is_meta_option parse_and_check_command_line parse_command_line
     preprocess_option_specs preprocess_option_specs_if_needed
     print_command_line_options_help print_current_option_settings
     print_invalid_options read_options_from_config_file);
...
use constant { META_OPTION => '.!' };

sub is_meta_option($) {
  my ($option) = @_;
  return starts_with($option, META_OPTION) ? 1 : 0;
}
...
use constant {
  ...
  OPTION_HELP_WRAPPED  => 2,
  OPTION_HELP_CATEGORY => 3,
  OPTION_HELP_SYNTAX   => 4,
  OPTION_HELP_DEFAULT  => 5,
};
...
sub print_command_line_options_help(+;+$) {
  ...
  my $header_colors = fg_color_rgb(96, 96, 96).U;
  ...
  @table_of_options = pairmap {
    ...
    if (is_array_ref $option) {
      ...
      if ($command == OPTION_HELP_LITERAL) {
        ...
        @rows = map { $_.NL } word_wrap_long_lines($description, $screen_width, '  ', '  ');
      } elsif ($command == OPTION_HELP_CATEGORY) {
        my ($tab_color, $tab_align) = @args;
        $tab_color //= SKYBLUE_2_3;
        $tab_align //= ALIGN_LEFT;

        @rows = (NL.print_folder_tab(C.$description, $tab_color, $tab_align).NL, $header_row);
      ...
      if (!defined $specs) 
        { die("Option '$option' listed in help but not defined in options spec"); }
      ...
      my $primary_alias = $shortest_aliases{$option};
      ...
      my $arg_help = ($action == OPTION_VALUE_REQUIRED) ? M_2_3.' '.double_left_angle_bracket.M.'arg'.M_2_3.double_right_angle_bracket :
        ($action == OPTION_VALUE_OPTIONAL) ? R_2_3.' '.wide_left_square_bracket.R.'arg'.R_2_3.wide_right_square_bracket.' (optional)' : '';
        ...
        ($type == SCALAR_REF) ? (
          ...
          is_numeric($default_as_scalar) ? (
            ...
            (($action == OPTION_ASSIGN_BOOL) || ($action == OPTION_TOGGLE_BOOL)) ? 
              (($default_as_scalar == 0) ? R.'no' : 
               ($default_as_scalar == 1) ? G.'yes' : 
               Y.$default_as_scalar) : 
             Y.$default_as_scalar) :
