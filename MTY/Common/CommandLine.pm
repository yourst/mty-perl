...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($command_line_option_re $key_value_option_re
     @default_config_file_alternatives INVALID_OPTION META_OPTION
     META_OPTION_HELP_DESCRIPTIONS META_OPTION_NON_OPTIONS_REQUIRED
     META_OPTION_PUT_ALL_HERE META_OPTION_PUT_FILENAMES_HERE
     META_OPTION_PUT_INVALID_ARGS_HERE NO_VAR_FOR_OPTION OPTION_ACTION_BASE
     OPTION_ACTION_COUNT OPTION_ACTION_MASK OPTION_APPEND_REPEATS
     OPTION_ASSIGN_BOOL OPTION_BOOL OPTION_COMMA_SEP_LISTS
     OPTION_DATA_TYPE_AUTO OPTION_DATA_TYPE_BASE OPTION_DATA_TYPE_COUNT
     OPTION_DATA_TYPE_MASK OPTION_FILE OPTION_FILE_WRITABLE OPTION_FLAGS_BASE
     OPTION_FLAGS_COUNT OPTION_FLAGS_MASK OPTION_FLOAT
     OPTION_HASH_MANIPULATION OPTION_HELP_CATEGORY OPTION_HELP_LITERAL
     OPTION_HELP_SYNTAX OPTION_INTEGER OPTION_MATCH_NAME_PREFIX
     OPTION_MATCH_NAME_SUBSTR OPTION_NO_SEPARATOR_BEFORE_VALUE
     ...
     OPTION_VALUE_OPTIONAL OPTION_VALUE_REQUIRED SINGLE_CHAR_OPTION_COMBINABLE
     SINGLE_CHAR_OPTION_DEFAULT SINGLE_CHAR_OPTION_NO_VALUE_SEP
     check_and_expand_prioritized_file_lists parse_and_check_command_line
...
sub preprocess_option_specs(+) {
  ...
  while (my ($name, $specs) = each %$origspecs) {
    ...
    if (starts_with($name, META_OPTION)) {
      # Flags or other parameters for parse_options (not a real user visible option):
      $expanded_specs{remove_from_start($name, META_OPTION)} = $specs;
      next;
    }
    ...
    if (is_scalar_typeid($vartype)) {
      ...
      die("Option '$name': spec references unsupported variable type (".ref($varref).")");
    }
...
use constant {
  OPTION_HELP_LITERAL  => 1,
  OPTION_HELP_CATEGORY => 2,
  OPTION_HELP_SYNTAX   => 3,
};
...
sub print_command_line_options_help(+;+$) {
  ...
  $help_strings //= [ map { $_ => undef } grep { $_ !~ /^\./oax } (sort keys %$options) ];
  my @option_names_or_categories_and_descriptions = pairmap { [ $a => $b ] } @$help_strings;

  my @table_of_options = ( );

  my $max_column_widths = [
    (maxlength(keys %$aliases) * 1.5) + 6,
    0,
    80
  ];

  my $option_alias_color = fg_color_rgb(192, 160, 80);
  ...
  while (my ($i, $name_desc_pair) = each @option_names_or_categories_and_descriptions) {
    my ($option, $description) = @$name_desc_pair;

    if (is_array_ref $description) {
      my ($command, @args) = @$description;
      if ($command == OPTION_HELP_LITERAL) {
        push @table_of_options, $option;
      } elsif ($command == OPTION_HELP_CATEGORY) {
        push @table_of_options, NL.print_folder_tab(C.$option, SKYBLUE_2_3, ALIGN_LEFT).NL;
      } elsif ($command == OPTION_HELP_SYNTAX) {
        my $s = print_folder_tab(Y.($option // 'Syntax').':', G, ALIGN_LEFT).NL.
          '  '.G.$0.K.' ['.M.'-option1 -option2 '.K.elipsis_three_dots.'] '.
            C.'file1 file2 '.K.elipsis_three_dots.NL;
        push @table_of_options, $s;
      } else {
    ...
    if (is_array_ref($varref) && ($flags & OPTION_COMMA_SEP_LISTS)) { $arg_help .= B.','.elipsis_three_dots; }

    push @line, $arg_help;

    push @line, X.($description // '');

    push @table_of_options, [ @line ];
  }

  my $out = format_table(@table_of_options, 
                         colseps => '  ', 
                         row_prefix => [ $dark_bgcolor.'  ', $light_bgcolor.'  ' ],
...
sub parse_and_check_command_line(+;++) {
  ...
  my $invalid = (scalar keys %$invalid_arg_indexes) || 
    ((exists $options->{META_OPTION_NON_OPTIONS_REQUIRED}) && (!scalar @$args)) ||
    $show_help;
