...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($command_line_option_re $convert_dash_to_stdin_or_stdout
     $key_value_option_re @default_config_file_alternatives INVALID_OPTION
     META_OPTION META_OPTION_DASH_TO_STDIO META_OPTION_HELP_DESCRIPTIONS
     META_OPTION_INCLUDE META_OPTION_MUTUALLY_EXCLUSIVE_OPTIONAL
     META_OPTION_MUTUALLY_EXCLUSIVE_REQUIRED META_OPTION_NON_OPTIONS_REQUIRED
     META_OPTION_ONE_OR_MORE_REQUIRED META_OPTION_PUT_ALL_HERE
     META_OPTION_PUT_FILENAMES_HERE META_OPTION_PUT_INVALID_ARGS_HERE
     ...
     OPTION_FLAGS_MASK OPTION_FLOAT OPTION_HASH OPTION_HASH_MANIPULATION
     OPTION_HELP_BANNER OPTION_HELP_BULLETED OPTION_HELP_CATEGORY
     OPTION_HELP_DEFAULT OPTION_HELP_LITERAL OPTION_HELP_SECTION
     OPTION_HELP_SPACER OPTION_HELP_SYNTAX OPTION_HELP_WRAPPED OPTION_INTEGER
     OPTION_LIST OPTION_MATCH_NAME_PREFIX OPTION_MATCH_NAME_SUBSTR
     OPTION_NO_SEPARATOR_BEFORE_VALUE OPTION_REGEXP
     OPTION_SINGLE_CHAR_COMBINABLE OPTION_SPEC_SLOT_ACTION_AND_FLAGS
     OPTION_SPEC_SLOT_ALIASES OPTION_SPEC_SLOT_DEF_VALUE_ON_RESET
     OPTION_SPEC_SLOT_DEF_VALUE_TO_SET OPTION_SPEC_SLOT_VARIABLE_REFERENCE
     OPTION_STRING OPTION_TOGGLE_BOOL OPTION_TOGGLE_BOOL_ON_EVERY_REPEAT
     OPTION_VALUE_OPTIONAL OPTION_VALUE_REQUIRED SINGLE_CHAR_NOT_AN_OPTION
     SINGLE_CHAR_OPTION_COMBINABLE SINGLE_CHAR_OPTION_DEFAULT
     SINGLE_CHAR_OPTION_NO_VALUE_SEP add_styles_to_description
     check_and_expand_prioritized_file_lists check_parsed_command_line
     include_options is_meta_option parse_and_check_command_line
     parse_command_line preprocess_option_specs
     preprocess_option_specs_if_needed print_command_line_options_help
     print_current_option_settings print_invalid_options
     read_options_from_config_file restore_option_settings_from_hash
     save_current_option_settings);
...
use constant enum (
  ...
  OPTION_SPEC_SLOT_ACTION_AND_FLAGS,
  OPTION_SPEC_SLOT_ALIASES,
  OPTION_SPEC_SLOT_DEF_VALUE_TO_SET,
  OPTION_SPEC_SLOT_DEF_VALUE_ON_RESET,
);
...
sub include_options(+) {
  my ($options) = @_;
  return (
    META_OPTION_INCLUDE.refaddr($options), 
    $options
  );
}
...
noexport:; sub preprocess_option_specs(+) {
  ...
  if (is_array_ref($origspecs)) { $origspecs = { @$origspecs }; };
  ...
  while (my ($name, $specs) = each %$origspecs) {
    ...
    $alias_list = [ sort { (length($a) <=> length($b)) } @$alias_list ] if ((scalar @$alias_list) > 1);

    if (is_scalar_ref_typeid($vartype)) {
      ...
      if (is_hash_ref($default_value_to_set)) {
        # automatically map strings to integer values:
        $default_value_to_reset //= invert_hash($default_value_to_set);
      } else {
      ...
      if (($action == OPTION_ASSIGN_BOOL) || ($action == OPTION_TOGGLE_BOOL))
        { die("Option '$name': ASSIGN_BOOL and TOGGLE_BOOL actions cannot be used on arrays"); }
      $default_value_to_set //= [ 1 ];
      $default_value_to_reset //= [ ];
    } elsif ($vartype == HASH_REF) {
      if (($action == OPTION_ASSIGN_BOOL) || ($action == OPTION_TOGGLE_BOOL))
        { die("Option '$name': ASSIGN_BOOL and TOGGLE_BOOL actions cannot be used on hashes"); }
      ...
      die("Option '$name': spec references unsupported variable type (".$typeid_to_string[$vartype // UNDEF], ' a.k.a. '.ref($varref).")");
    }
  ...
  if ((!exists $aliases->{'help'}) && (!hash_contains_all_keys($aliases, qw(h e l p)))) {
  ...
  $expanded_specs{META_OPTION.'aliases'} = $aliases;
  ...
  $expanded_specs{META_OPTION.'single-char-option-hints'} = $single_char_option_hints;    
...
sub preprocess_option_specs_if_needed(+) {
  ...
  if (exists $specs->{META_OPTION.'aliases'}) 
    { return ($specs, $specs->{META_OPTION.'aliases'}, $specs->{META_OPTION.'single_char_option_hints'}); }
...
sub add_styles_to_description($) {
  ...
  $description =~ s{(?<! [\w\-]) -([^\s\,\;]+)}{Y_1_2.'-'.Y.$1.X}oamsxge;
  return $description;
}
...
# my $header_bgcolor = bg_color_rgb(24, 0, 64);
  ...
  my @table_of_options = pairmap {
    ...
    if (is_array_ref $option) {
      ...
      if ($command == OPTION_HELP_LITERAL) {
        ...
        @rows = map { $_.NL } (word_wrap_long_lines(add_styles_to_description($description), $screen_width, '  ', '  '));
        $already_added_header_row = 0;
      } elsif ($command == OPTION_HELP_BULLETED) {
        my ($bullet) = @args;
        $bullet //= B.dot;
        @rows = map { $_.NL } (word_wrap_long_lines(add_styles_to_description($description), $screen_width, '  '.$bullet.X.' ', '    '));
        $already_added_header_row = 0;
        ...
        @rows = ( NL.print_folder_tab(C.$description, $tab_color, $tab_align).NL );
        $already_added_header_row = ($command == OPTION_HELP_SECTION) ? 1 : 0;
      } elsif ($command == OPTION_HELP_BANNER) {
        my ($box_color, $title) = @args;
        $box_color //= B;
        $title //= filename_of($0);
        @rows = ( print_banner($title, $description // '', $box_color) );
...
sub check_parsed_command_line(++++) {
  my ($options, $filenames, $invalid_arg_indexes, $option_names_to_values) = @_;
...
sub parse_and_check_command_line(+;++) {
  ...
  my $invalid = (!check_parsed_command_line($options, $filenames, 
    $invalid_arg_indexes, $option_names_to_values));
...
sub check_and_expand_prioritized_file_lists {
  my $list_of_lists = (is_array_ref $_[0]) ? \@_ : [ @_ ];
  my @out = ( );

  my $program = filename_of($0);
  if ($program eq '-e') { $program = undef; }

  foreach my $path_or_list (@$list_of_lists) {
    next if (!defined $path_or_list);
    my $list = (is_array_ref $path_or_list) ? $path_or_list : [ $path_or_list ];

    foreach my $filename (@$list) {
      next if (!defined $filename);

      my $f = (defined $program) ? 
        ($filename =~ s{$placeholder_re}{$program}roamsxg) : $filename;

      if (path_exists($f)) { push @out, $f; last; }
    }
  }
...
sub read_options_from_config_file {
  my $alternatives = (scalar @_) ? \@_ : \@default_config_file_alternatives;
