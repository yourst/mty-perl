...
use Scalar::Util qw(refaddr reftype looks_like_number openhandle isdual);
...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($empty_scalar $undef_scalar $zero_scalar %empty_hash
     %ref_type_string_to_index @empty_array @ref_type_index_to_string
     @ref_type_index_to_symbol ARRAY_REF BILLION CODE_REF EB FORMAT_REF GB
     GLOB_REF HASH_REF INT_MAX IO_REF KB LONG_MAX LVALUE_REF MB MILLION
     QUADRILLION RATIO_1_10 RATIO_1_2 RATIO_1_3 RATIO_1_4 RATIO_2_3 RATIO_3_4
     RATIO_9_10 REF_REF REGEXP_REF SCALAR SCALAR_REF TB THOUSAND TRILLION
     VSTRING_REF all_defined all_empty all_filled all_there any_empty
     any_filled any_there any_undefined bit bitmask bits clipto
     compare_arrays compare_vec_to_scalar_and_generate_bitmask
     copy_array_elements_where_undef extractbits first_specified
     hash_of_constants_in_package inrange is_array_ref is_code_ref
     is_constant is_empty is_filled is_hash_ref is_numeric is_ref is_ref_ref
     is_regexp_ref is_scalar is_scalar_ref is_string is_there list_contains
     list_of_constants_in_package lowbits masked_store_into_array max
     max_in_list min min_in_list pad_array pair_list_with_value
...
sub typeof(+) {
  my $t = $ref_type_string_first_char_to_index[ord(ref $_[0])];
  return $t if ($t != REGEXP_REF);
  return (re::is_regexp($_[0])) ? REGEXP_REF : REF_REF;
}
