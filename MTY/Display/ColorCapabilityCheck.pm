...
nobundle:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(NOT_COLOR_CAPABLE is_fd_on_terminal stdin_is_terminal ANSI_COLOR_CAPABLE
     colorize_debug_log stderr_is_terminal stdout_is_terminal
     get_console_control_fd is_stderr_color_capable is_stdout_color_capable
     is_console_color_capable ENHANCED_RGB_COLOR_CAPABLE
     is_filehandle_color_capable @color_capable_interactive_viewers
     %consumer_command_color_capabilities %terminal_name_to_color_capabilities
     @disable_color_when_piped_to_these_consumers);
...
my @is_fd_on_terminal_cache;

sub is_fd_on_terminal($) {
  my ($fd) = @_;
  my $is_terminal = $is_fd_on_terminal_cache[$fd];
  if (defined $is_terminal) { return $is_terminal; }

  $is_terminal = (isatty($fd)) ? 1 : 0;
  if (($fd == STDIN_FD) || ($fd == STDOUT_FD) || ($fd == STDERR_FD)) {
    # Only cache the results from the stdin/stdout/stderr file descriptors,
    # since these typically remain open throughout the program's execution:
    $is_fd_on_terminal_cache[$fd] = $is_terminal;
  }
  return $is_terminal;
}

sub stdin_is_terminal() { is_fd_on_terminal(STDIN_FD); }
sub stdout_is_terminal() { is_fd_on_terminal(STDOUT_FD); }
sub stderr_is_terminal() { is_fd_on_terminal(STDERR_FD); }
