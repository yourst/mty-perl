...
nobundle:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(print_ppi_tree flatten_ppi_tree odd_line_bg_color even_line_bg_color
     colored_equals_sign dump_printable_tree process_ppi_tree_common
     convert_ppi_tree_to_printable_tree $ppi_tree_formatter_simplify_output
     convert_ppi_subtree_to_printable_tree);
...
use constant {
  colored_equals_sign => (X.K.' = '.X),
  even_line_bg_color => bg_color_rgb(32, 32, 32),
  odd_line_bg_color => bg_color_rgb(20, 20, 20),
};
...
noexport:; sub convert_ppi_subtree_to_printable_tree {
  ...
  if ($nodetype =~ /^ token \s*+ (.+) $/oax) {
    ...
    $nodetype = $G.((if_there $1) // $nodetype);
    $omit_quotes = 1;
  } elsif ($nodetype =~ /regexp/oax) {
    $nodetype = ORANGE.$nodetype;
    $text_color = ORANGE;
    $omit_quotes = 1;
    ...
    $nodetype = $C.$start.' '.$U.((if_there $1) // $nodetype).$UX.' '.$finish;
    $omit_quotes = 1;
    $omit_equals = 1;
  ...
  my $info = [
    [ 
      ...
      even_line_bg_color.$line_prefix.$col_prefix.' '.X,  
      odd_line_bg_color.$line_prefix.$col_prefix.' '.X
    ],
    $nodetype,
    (is_there($data) ? ($omit_equals ? ' ' : colored_equals_sign) : ''),
    $data // '',
    X,
    [ TREE_CMD_SYMBOL, undef ],
  ];
  ...
  my $simplify_output = 1; #$ppi_tree_formatter_simplify_output;
  ...
  foreach my $subnode (@$subnodes) {
    ...
    if ($simplify_output && (defined $prev_printable_tree_node_info) && ($subtype =~ /operator/) && 
          ($subdata eq ',' || $subdata eq '=>' || $subdata eq '.') &&
            (scalar(@$r)) == 1) {
      # don't reprint the line and column info: use the first node's info
      push @$info, [ TREE_CMD_SYMBOL, three_vert_bars ];
      # use DDP; p @$subinfo;
      splice(@$subinfo, 0, 6);

      if ($prev_printable_tree_node_info->[2] eq colored_equals_sign) 
        { $prev_printable_tree_node_info->[2] = X; };

      push @{$prev_printable_tree_node_info},
        M_1_2.' '.dashed_vert_bar_3_dashes.' ',
        @{$subinfo};
    ...
    } elsif ($simplify_output &&
               (defined $prev_printable_tree_node) && 
               ($subtype eq $prev_printable_tree_node_info->[0]) &&
               (scalar(@$prev_printable_tree_node) == 1) &&
               (scalar(@$r) == 1)) { 
      # don't reprint the line and column info: use the first node's info
      push @$info, [ TREE_CMD_SYMBOL, three_vert_bars ];
      #printfd(STDERR, 'pptni = [ "', join('", "', @$prev_printable_tree_node_info), '" ]', NL);
      #printfd(STDERR, 'subinfo = [ "', join('", "', @$subinfo), '" ]', NL);

      splice(@{$subinfo}, 1, 5);
      push @{$prev_printable_tree_node_info},
        M_1_2.' '.dotted_vert_bar_4_dots.' ',
        $subinfo->[2];
      $prev_printable_tree_node_info->[1] = $special_line_and_col_bg_color;
    } else {
