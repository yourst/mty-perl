...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($B $C $G $K $M $Q $R $U $V $W $X $Y $UX $VX EOE $bgB $bgC $bgG $bgK $bgM
     $bgQ $bgR $bgW $bgY $BLINK $BLINKX COLOR_B COLOR_C COLOR_G COLOR_K
     COLOR_M COLOR_Q COLOR_R COLOR_W COLOR_Y bg_gray fg_gray colorize
     blend_rgb luminance scale_rgb CLEAR_LINE END_OSC_ST %rgb_colors
     CLEAR_SCREEN bg_color_rgb blend_rgb_bg blend_rgb_fg clear_screen
     enable_color fg_color_rgb scale_rgb_bg scale_rgb_fg %fixed_colors
     disable_color set_tab_stops $color_enabled %custom_colors SET_XTERM_FONT
     CLEAR_TAB_STOPS SAVE_CURSOR_POS clear_tab_stops ANSI_CONSOLE_ESC
     ANSI_CONSOLE_OSC adjust_luminance scale_rgb_values %rgb_color_values
     color_sample_text rgb_of_color_code set_console_title %color_style_codes
     %scaled_rgb_colors @all_color_symbols CLEAR_REST_OF_LINE
     RESTORE_CURSOR_POS get_tab_stop_codes move_cursor_to_row
     replace_color_code ANSI_CONSOLE_SET_BG ANSI_CONSOLE_SET_FG
     %custom_color_values @fixed_color_symbols set_console_subtitle
     set_rgb_bg_in_string %scaled_custom_colors @custom_color_symbols
     SET_ANSI_COLOR_TO_RGB move_cursor_to_column scale_rgb_fg_in_array
     set_console_attribute SET_XTERM_WINDOW_TITLE blend_and_scale_rgb_bg
     blend_and_scale_rgb_fg blend_rgb_bg_in_string blend_rgb_fg_in_string
     scale_rgb_bg_in_string scale_rgb_fg_in_string CLEAR_LINE_UP_TO_CURSOR
     SET_XTERM_SESSION_TITLE %color_markup_to_code_map
     SET_TAB_IN_CURRENT_COLUMN update_terminal_dimensions
     color_and_style_sample_text enable_color_based_on_stderr
     enable_color_based_on_stdout get_terminal_height_in_lines
     rgb_of_color_code_components enable_color_based_on_console
     generate_scaled_rgb_constants get_terminal_width_in_columns
     move_cursor_to_row_and_column %basic_color_markup_to_code_map
     reveal_ansi_console_escape_code blend_and_scale_rgb_bg_in_string
     blend_and_scale_rgb_fg_in_string enable_color_based_on_filehandle
     reveal_ansi_console_escape_codes scale_rgb_fg_in_array_recursively
     %enhanced_color_markup_to_code_map __scale_rgb_fg_in_array_recursively);

# For convenience, re-export these from MTY::Display::ColorCapabilityCheck:
push @EXPORT, qw(is_fd_on_terminal stdin_is_terminal stdout_is_terminal stderr_is_terminal);
...
use constant {
  ...
  EOE => 'm', # end of escape sequence
  END_OSC_ST => chr(007), # string terminator (for escapes that accept a string)
};
...
use constant {
  ...
  CLEAR_REST_OF_LINE => ANSI_CONSOLE_ESC.'K',
  CLEAR_LINE_UP_TO_CURSOR => ANSI_CONSOLE_ESC.'1K',
  CLEAR_LINE => ANSI_CONSOLE_ESC.'2K',
...
BEGIN {
  ...
  %custom_color_values = (
    ...
    PURPLE => [172, 80, 255],
    SKYBLUE => [64, 192, 255],
    LIGHTBLUE => [170, 154, 255],
    YELLOW_GREEN => [176, 255, 64],
  );
...
sub scale_rgb(+$;$) {
  ...
  if (!defined $rgb) { use Carp; print(Carp::longmess); die;}
