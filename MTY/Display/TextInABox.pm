...
noexport:; sub fg_bg_and_rgb_hex_or_list_to_codes($$$$$) {
  ...
  if (defined $hex_rgb) { ($r, $g, $b) = parse_rgb_hex_triplet($hex_rgb); }
...
use constant {
  TAB_STRAIGHT_BOTTOM_EDGES  => (1 << 0),
  TAB_STRAIGHT_TAB_EDGES     => (1 << 1),
  TAB_ONE_LINE_OVERLAY_LABEL => (1 << 2),
  # (only used internally):
  TAB_IS_ATTACHED_TO_BOX     => (1 << 31), 
};
...
sub print_folder_tab($;$$$$$$$$) {
  my ($label, $outline_color, $alignment, $width, $style_name, $flags, $prefix, $suffix, $dots_under_label_color) = @_;
...
sub print_banner($$;$$) {
  ...
  my $message =
    ...
    '%{div}'.NL.
    $X.'Copyright '.copyright_symbol.' 2003-2014 '.$Y.'Matt T. Yourst '.$X.'<yourst@yourst.com>'.$X.NL.
    $K.'This program is free software licensed under GPL v3.0'.NL.
    ($_[3] // '');
