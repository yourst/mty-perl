...
my $supported_source_types = {
  c => {
    ...
    suffixes    => [ 'c' ],
    emacs_modes => [ 'c' ],
    tree_node_symbol => C.roman_numeral_c.'  ',
  },
  ...
  c_preproc => {
    ...
    suffixes    => [ 'i' ],
    emacs_modes => [ 'c' ],
    tree_node_symbol => R.p_in_circle.roman_numeral_c.' ',
  },
  ...
  cpp => { 
    ...
    emacs_modes => [ 'c++' ],
    tree_node_symbol => G.roman_numeral_c.superscript_plus.superscript_plus,
  }, 
  ...
  cpp_header => {
    ...
    emacs_modes => [ 'c++.h' ],
    tree_node_symbol => Y.italic_lowercase_h.superscript_plus.superscript_plus,
  },
  ...
  cpp_preproc => { 
    ...
    emacs_modes => [ 'c++'],
    tree_node_symbol => B.p_in_circle.roman_numeral_c.superscript_plus.superscript_plus,
  },
};

my $no_default_includes;
...
method:; sub new($$$;$$) { # (constructor)
  ...
  # printdebug($filename, ': parent ', ((defined $parent) ? $parent->{filename} : '<none>'),
  #   ', parent_is_cxx? ', $parent_is_cxx, ', is_cxx? ', $is_cxx);
  ...
  while ($code =~ /$c_cxx_included_deps_re/oamsxg) {
    ...
    # (.g. printf("... this is a string with #include <xyz.h> inside of it"),
    ...
    if ($directive eq 'include') {
      ...
      if ($argument eq 'end_of_includes' || $argument eq 'end_of_headers') { last; }
      elsif ($argument eq 'once') { $is_header = 1; }
    } elsif ($directive eq 'if') {
      my $ok = (exists $defines->{$argument});
  ...
  my $type_name = 
    ($is_preprocessed) ? ($is_cxx ? 'cpp_preproc' : 'c_preproc') :
    ($is_cxx) ? ($is_header ? 'cpp_header' : 'cpp') :
    ($is_header ? 'c_header' : 'c');

  $type = $supported_source_types->{$type_name};
...
noexport:; sub get_default_c_cxx_compiler_include_paths(+) {
  ...
  if (defined $cache) {
    ...
    printdebug('Cache provided the following C/C++ settings for ', $c_cxx_compiler, ' compiler:', NL,
               ...
               'cxx_defines = ', join(' ', sort keys %$c_defines));
    return;
  }
  ...
  if ($compiler_c_output =~ /$compiler_include_paths_re/oamsxg) {
    ...
    printfd(STDERR, $Y.$U.'ERROR: Could not find default C include paths in compiler output:'.
