...
preserve:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(SOURCE_FLAG_BINARY SOURCE_FLAG_COMPILABLE SOURCE_FLAG_INCLUDABLE 
     ...
     determine_file_type read_source_and_instantiate_by_type filename_to_source
...
export:; sub read_source_and_instantiate_by_type($;+$) {
  ...
  #printfd(STDERR, 'set f2s{', $filename, '} = ', $source, NL);
  ...
  foreach my $symdep (@$symdeps) {
    ...
    if (!defined $filename) {
      warning("Cannot resolve symbolic dependency \"$symdep\" into filename; ".
                "assuming \"$thisfile\" does not actually depend on it");
      next;
    }
    $filedeps{$filename} = 1;
  }
...
sub get_deps_by_type(+$) :method {
  ...
  return 
    ...
    ($deptype == ALL_DEPS) ? $this->{recursive_deps} :
    ($deptype == INVERSE_DEPS) ? $this->{inverse_deps} : 
    $this->{direct_symbolic_deps};
}

sub condense_filename(+) :method {
  return format_filename_with_path_prefix($_[0]->{filename});
}

sub format_deps(+$$) :method {
  my ($this, $deptype, $format) = @_;

  my $deps = $this->get_deps_by_type($deptype);

  my @deplist = map { 
    my $filename = filename_of_source($_);
...
sub reconstruct_after_retrieve(+) :method {
  ...
  my $type = $this->{type};
  my $typespec = $source_type_to_spec{$type};
...
sub prepare_to_store_sources_to_cache_file(;+) {
  ...
  if (is_debug) {
    foreach my $source (@sources) {
      printfd(STDERR, NL, print_folder_tab(R.U.'BEFORE prep for store:'.UX.Y.' '.$source->{filename}), NL);
      pp($source, $source->{filename});
    }
  }
...
sub reconstruct_sources_after_retrieving_from_cache_file(+) {
  ...
  foreach my $source (@$cached_sources) {
    ...
    if (!defined $source) {
      printfd(STDERR, "Metadata for $filename could not be retrieved ".
  ...
  if (is_debug) {
    foreach my $source (@sources) {
      printfd(STDERR, NL, print_folder_tab(G.U.'AFTER retrieve:'.UX.Y.' '.$source->{filename}), NL);
      pp($source, $source->{filename});
    }
  }
...
sub configure($++;+) :method {
  ...
  reconstruct_sources_after_retrieving_from_cache_file($cache) if (defined $cache);
}
