...
nobundle:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($enum_def_re $c_comment_re $cxx_space_re $cxx_token_re $type_spec_re
     %cxx_keywords $cxx_quoted_re $field_decl_re $ptr_or_ref_re
     $cxx_comment_re $cxx_keywords_re $preprocessor_re $typedef_decl_re
     CXX_TOKEN_QUOTED $c_cxx_comment_re $cxx_operators_re $cxx_opt_space_re
     $identifier_chars $template_spec_re @cxx_keyword_list CXX_TOKEN_COMMENT
     CXX_TOKEN_NUMERIC $cxx_identifier_re $looks_like_cxx_re
     CXX_TOKEN_OPERATOR $non_identifier_syms CXX_TOKEN_IDENTIFIER
     CXX_TOKEN_STRUCTURAL CXX_TOKEN_WHITESPACE $cxx_only_keywords_re
     $cxx_quoted_string_re $if_or_ifdef_endif_re $rvalue_expression_re
     %cxx_operator_to_name $class_struct_union_re $template_type_list_re
     CXX_TOKEN_PREPROCESSOR $c_cxx_included_deps_re $preprocessor_define_re
     $declaration_or_block_re @all_capture_group_names
     $cxx_extended_operators_re $preprocessor_line_info_re
     $cxx_assignment_operators_re $cxx_double_quoted_string_re
     $cxx_quoted_literal_nocap_re $cxx_raw_delimited_string_re
     $cxx_single_quoted_string_re $cxx_preprocessor_directive_re
     $cxx_unary_prefix_operators_re $standalone_type_spec_nocap_re
     $cxx_single_quoted_character_re $cxx_unary_postfix_operators_re
     $function_type_name_args_body_re %cxx_assignment_operator_to_name
     $cxx_balanced_structural_chars_re $strip_non_functional_cxx_syntax_re
     $cxx_preprocessor_directive_names_re $function_decl_argument_list_item_re
     $cxx_extended_assignment_operators_re
     $c_cxx_include_guard_or_pragma_once_re
     $cxx_preprocessor_include_directive_re
     $inside_if_or_ifdef_optional_else_endif_re
     $cxx_binary_arith_logic_compare_operators_re
     $cxx_preprocessor_define_directive_arguments_re
     $cxx_preprocessor_include_directive_arguments_re
     $cxx_assignment_operators_except_simple_equals_re
