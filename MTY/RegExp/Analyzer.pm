...
#
# Copyright 2005 - 2015 Matt T. Yourst <yourst@yourst.com>
#
...
nobundle:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(colorize_regexp_text format_metadata_line show_compiled_regexp
     beautify_regexp_token list_compiled_regexps show_compiled_regexps
     describe_regexp_metadata show_raw_compiled_regexps
     short_list_compiled_regexps $show_pretty_compiled_regexps
     format_regexp_parse_tree_node simple_format_tokenized_regexp
     $show_colorized_compiled_regexps regexp_parse_tree_to_printable_tree
     $use_enhanced_regexp_unicode_symbols
     regexp_parse_subtree_to_printable_subtree
     convert_perl_regexp_to_boost_xpressive_regexp);
...
my $tree_cmd_prefix_default_bg_color =
  [ TREE_CMD_EVEN_ODD_PREFIX, bg_color_rgb(32, 32, 32), bg_color_rgb(20, 20, 20) ];
...
#    $token_sym_color.arrow_open_tri;
  ...
  my $label = [
    ...
    # Prefix to left of branches
    $tree_cmd_prefix_default_bg_color,
    [ TREE_CMD_PREFIX, fg_color_rgb(64,  96, 128).' '.padstring($inputpos // R.'???', -4).
        ' '.$DARKGRAY.long_narrow_double_vert_bars.' '.
          padstring($token_symbol, 2).' '.$token_color.UX.padstring($typename, 9).X,
    ],
...
sub list_compiled_regexps() {
  my $table = [ ];
  my $description_color = fg_color_rgb(152, 192, 208); # desaturated light blue

  foreach my $name (sort keys %compiled_regexps) {
    my $description = $compiled_regexp_descriptions{$name} // '';
    push @$table, [
      Y.$name.'_re'.X,
  ...
  prints(format_table($table, wrap_above_max_col_widths => [ 0, WRAP_TO_REMAINING_SPACE_ON_LINE ]));
}
...
sub show_compiled_regexp($;$$$) {
  ...
  #
  my $printable_original_regexp = colorize_regexp_text($resrc);
