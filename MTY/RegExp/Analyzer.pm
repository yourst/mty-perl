...
nobundle:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw($show_colorized_compiled_regexps $show_pretty_compiled_regexps
     ...
     die_here format_metadata_line format_regexp_parse_tree_node
     format_tokenized_regexp list_compiled_regexps
...
sub beautify_regexp_token($$+;$$$$) {
  ...
  # local (*token, *type, *subparts, *cap_group_index, *append_to_comment, *keep_orig_spacing, *hide_implied_backslashes) = \ (@_);
...
my %token_type_to_token_symbol = (
  RE_TOKEN_CAP_GROUP_START,             $C.checkmark, #$C.round_bold_left_paren.round_bold_right_paren, #$C.round_bold_left_paren.round_bold_right_paren,
  RE_TOKEN_EXT_GROUP_START,             $B.box_with_shadow,    #$B.bold_left_brace.bold_right_brace,
  RE_TOKEN_LITERAL,                     $G.left_quote.right_quote,
  RE_TOKEN_LITERAL_STRING,              $C.left_quote.right_quote,
  RE_TOKEN_LITERAL_NUMERIC,             $R.left_quote.right_quote,
  RE_TOKEN_ANY_CHAR,                    $Y.asterisk,
  RE_TOKEN_ESCAPED_CHAR_CLASS,          $Y.degrees_c,
  RE_TOKEN_SPACE_CHAR_CLASS,            $MID_GRAY.under_space,
  RE_TOKEN_BRACKETED_CHAR_SET,          $M.small_upper_left_corner_bracket.small_lower_right_corner_bracket,
  RE_TOKEN_ANCHOR,                      $M.anchor_symbol,
  RE_TOKEN_BACKREF,                     $R.left_angle_quote.left_angle_quote,
  RE_TOKEN_OR,                          $R.four_diamonds,
  RE_TOKEN_COMMENT,                     $K.sharp_sign,
  RE_TOKEN_CONTROL,                     $R.warning_sign,
);

my %token_type_to_node_symbol = (
  RE_TOKEN_OR,                          $R.four_diamonds,
  RE_TOKEN_CAP_GROUP_START,             $C.checkmark_in_box,
  RE_TOKEN_EXT_GROUP_START,             $B.arrow_tri,
);
...
sub format_regexp_parse_tree_node(+;+) {
  ...
  my $node_symbol = ($token_type_to_node_symbol{$type} // $branch_color.arrow_open_tri);
  my $token_symbol = ($token_type_to_token_symbol{$type} // '  ');
  ...
  my $label = [
    ...
    [ TREE_CMD_SYMBOL, $node_symbol, $node_symbol ],
    [ TREE_CMD_PREFIX, bg_color_rgb(32, 32, 32), bg_color_rgb(20, 20, 20) ],
    [ TREE_CMD_PREFIX, fg_color_rgb(64,  96, 128).' '.
        padstring($inputpos // $R.'???', -4).' '.$X.$DARKGRAY.
          long_narrow_double_vert_bars.' ' ],
    [ TREE_CMD_PREFIX, padstring($token_color.$UX.$typename.$X.$DARKGRAY, 10, -1, dot_small).' ' ],
    [ TREE_CMD_PREFIX, $token_symbol ],
    $token_color.(($type == RE_TOKEN_COMMENT) ? $K.'(comment)' : $beautified_token).$X,
    [ TREE_CMD_FIELD ],
    [ TREE_CMD_FIELD ],
    padstring($quantifier_and_greed, 30),
    (($type == RE_TOKEN_COMMENT) 
       ? $K.' '.long_narrow_vert_bar.' '.($token =~ s/^\#\s*//roamsxg)
       : $B.' '.long_narrow_vert_bar.' '.$comment)
  ];
...
sub format_tokenized_regexp {
  my ($tokens, $single_line) = @_;
  # local (*tokens, *single_line) = \ (@_);
  ...
  my $enclosing_group_type = RE_TOKEN_UNKNOWN;
  my @enclosing_group_stack = (RE_TOKEN_UNKNOWN);

  my $prev_type = RE_TOKEN_UNKNOWN;
  my $cap_group_index = 1;
  my $comment = '';
  my $linebuf = '';

  my $cap_group_count = 0;
  my $named_cap_group_count = 0;
  my $ext_group_count = 0;

  foreach my $token (@{$tokens}) {
    my ($token, $type, $startpos, $len, $subparts) = @{$token};
    #print(STDERR '>> type ['.($type // '???').' => '.($regexp_token_type_to_name{$type} // '???').'] @ '.($startpos // '???').': "'.($token // '???').'"'.NL);
    $cap_group_count += ($type == RE_TOKEN_CAP_GROUP_START);
    $named_cap_group_count += (exists($subparts->{name}));
    $ext_group_count += ($type == RE_TOKEN_EXT_GROUP_START);
  }

  if ($single_line) {
    $out .= $K.'['.
      $C.$cap_group_count.$K.' caps, '.
      $C.$named_cap_group_count.$K.' named, '.
      $B.$ext_group_count.$K.' groups'.
      $K.']: ';
  } else {
    $out .= $K.'regexp comprises '.
      $C.$cap_group_count.$Y.' capture groups'.$K.', '.
      $C.$named_cap_group_count.$Y.' named cap groups'.$K.', '.
      $B.$ext_group_count.$Y.' ext groups'.$K.' and '.
      $G.$n.$Y.' tokens'.$K.':'.
      $X."\n\n";
  }

  my $orig_chars_so_far = 0;
  my $prev_indent = 0;
  
  for (my $i = 0; $i < $n; $i++) {
    my $t = $tokens->[$i];
    my ($token, $type, $startpos, $len, $subparts) = @{$t};
    my $typename = $regexp_token_type_to_name{$type} // '<type #'.$type.'>';
    my $first_char = substr($token,1,1);
    my $token_is_group_start = ($type == RE_TOKEN_CAP_GROUP_START || $type == RE_TOKEN_EXT_GROUP_START);
    my $token_is_group_end = ($type == RE_TOKEN_GROUP_END);

    if ($token_is_group_end) { 
      $indent--;
    ...
    if ($token_is_group_end) {
      $type = 
        ($enclosing_group_type == RE_TOKEN_CAP_GROUP_START) ? RE_TOKEN_GROUP_END :
        ($enclosing_group_type == RE_TOKEN_EXT_GROUP_START) ? RE_TOKEN_GROUP_END :
        $type;
    } elsif ($type == RE_TOKEN_OR) {
      $linebuf .= ' ';
    } elsif ((($type == RE_TOKEN_LITERAL) && ($prev_type == RE_TOKEN_LITERAL)) || ($type == RE_TOKEN_QUANTIFIER)) {
      $linebuf .= '';
    } elsif ($single_line) {
      $out .= $linebuf;
    } else {
      if ($i > 0) {
        $out .= padstring($linebuf, 80);
        $linebuf = '';
        if (is_there($comment)) {
          $out .= $B.dashed_vert_bar_3_dashes.' '.$K.$comment.$X;
          $comment = '';
        }
        $out .= "\n";
      }
      ...
      if ($type == RE_TOKEN_UNKNOWN) {
        my $chars_left = $len;
        $out .= (' ' x 19) . ' '.$R.x_symbol.' '.$Y.$U.'WARNING:'.$X.$R.
          ' syntax error in regexp at or before character index '.
            $Y.$startpos.$R.' (token '.$Y.'#'.$i.$R.'), with '.
              $Y.$chars_left.$R.' characters remaining:'.$X."\n";
      }

      my $type_color = $token_type_to_formatting{$type} // $Y;

      my $new_indent = ($token_is_group_start) ? ($indent+1) : $indent;
      my $indent_change = $new_indent - $prev_indent;
      $indent_change = +1 if ($token_is_group_start);
      $prev_indent = $new_indent;
      my $pos_color = ($type == RE_TOKEN_UNKNOWN) ? $R : $Y;
      $linebuf .= sprintf($pos_color.'%4d'.' '.$B.long_narrow_vert_bar.' '.
                          (($indent_change > 0) ? ($G.arrow_tri) : 
                           ($indent_change < 0) ? ($R.left_arrow_tri) : 
                           ($K.' ')).' '.#($K.$down_arrow_open_tri)).' '.
                          ((!$indent_change) ? '  ' : ($indent < 10) ? (enclosed_digit($indent+1, circled_large_digits).' ') : sprintf('%-2d', $indent+1)).
                          '  '.$type_color.'%-15s'.$B.'  '.
                          long_narrow_vert_bar.' ', $startpos, $typename);
      
      $linebuf .= $B.((bullet_small.' ') x $indent);
    }

    my $formatted_token = beautify_regexp_token($token, $type, $subparts, $cap_group_index, \$comment);
    $linebuf .= $formatted_token;

    if ($type == RE_TOKEN_CAP_GROUP_START) { $cap_group_index++; }

    if ($token_is_group_start) { 
      $indent++;
      if ($indent > 40) { $indent = 40; }
      push @enclosing_group_stack,$enclosing_group_type;
      $enclosing_group_type = $type;
    } elsif ($token =~ /^\)/oamsxg) { 
      $enclosing_group_type = pop @enclosing_group_stack;
      die if (!defined($enclosing_group_type));
    }

    $orig_chars_so_far += length($token);
    $prev_type = $type;
  }
  $out .= $linebuf;
  if (!$single_line) { $out .= NL; }
