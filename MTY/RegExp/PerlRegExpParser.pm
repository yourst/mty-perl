...
nobundle:; our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(RE_TOKEN_OR RE_TOKEN_ANCHOR RE_TOKEN_REGEXP tokenize_regexp
     $regexp_token_re RE_TOKEN_BACKREF RE_TOKEN_COMMENT RE_TOKEN_CONTROL
     RE_TOKEN_LITERAL RE_TOKEN_UNKNOWN RE_TREE_NODE_POS $regexp_anchor_re
     @greedyness_names RE_TOKEN_ANY_CHAR RE_TOKEN_CASE_MOD RE_TREE_NODE_TYPE
     $regexp_backref_re $regexp_comment_re $regexp_control_re
     RE_TOKEN_GROUP_END RE_TREE_NODE_TOKEN $regexp_any_char_re
     $regexp_case_mod_re INFINITE_QUANTIFIER RE_TOKEN_QUANTIFIER
     RE_TOKEN_WHITESPACE RE_TREE_NODE_FIELDS $redundant_escape_re
     $regexp_group_end_re $regexp_modifiers_re RE_GREEDYNESS_GREEDY
     $regexp_quantifier_re RE_GREEDYNESS_MINIMAL RE_TREE_NODE_GROUP_ID
     RE_TREE_NODE_SUBNODES $any_escaped_symbol_re RE_TREE_NODE_MAX_QUANT
     RE_TREE_NODE_MIN_QUANT dump_regexp_parse_tree $regexp_literal_char_re
     RE_TOKEN_LITERAL_STRING RE_TREE_NODE_GREEDYNESS tokenize_regexp_to_tree
     RE_GREEDYNESS_POSSESSIVE RE_TOKEN_CAP_GROUP_START
     RE_TOKEN_EXT_GROUP_START RE_TOKEN_LITERAL_NUMERIC
     tokenized_regexp_to_tree $regexp_alternation_or_re
     $regexp_literal_string_re RE_TOKEN_SPACE_CHAR_CLASS
     $regexp_cap_group_start_re $regexp_ext_group_start_re
     %regexp_token_type_to_name $regexp_space_char_class_re
     RE_TOKEN_BRACKETED_CHAR_SET RE_TOKEN_ESCAPED_CHAR_CLASS
     tokenized_regexp_to_subtree $regexp_bracketed_char_set_re
     $regexp_escaped_char_class_re $regexp_quantifier_labeled_re
     @regexp_tree_node_field_names check_for_regexp_parse_errors
     get_tokenized_regexp_metadata $regexp_numeric_literal_char_re
     $regexp_tokenizer_skip_whitespace $regexp_bracketed_char_set_nocap_re
     clean_up_and_optimize_regexp_parse_tree);
...
our $regexp_bracketed_char_set_nocap_re = compile_regexp(
  qr{\[ \^? \]? (?> \\ . || [^\]])++ \]}oamsx, 'regexp_bracketed_char_set_nocap');
...
our $regexp_literal_string_re = compile_regexp(
  qr{(?> (?! \\) \\ Q (?> [^\\] | \\ [^E])*+ \\ E)}oamsx, 'regexp_literal_string');
...
our $regexp_modifiers_re = compile_regexp(
  qr{(?> \^? [adlupimsx]*+ (?> - [imsx]++)?)}oamsx,
  'regexp_modifiers', 'Modifier character sequences in regexps of the form '.
  '(?modifiers:...) to only affect the inner group contents (...) or '.
  '(?modifiers) to affect the remainder of the containing group.');

our $regexp_ext_group_start_re = compile_regexp(
  qr{\( \? (?:
     (?'group_type' [R\:\=\!\>\|]) |
     (?> (?'modifiers' $regexp_modifiers_re) (?'group_type' \: | (?= \)))) |
     (?'group_type' (?: \< [\=\!])) |
     (?'group_type' [\#\{\(]) |
     (?'group_type' (?: \? \{ )) |
...
sub tokenize_regexp($;$$$) {
  ...
  while ($re =~ /$regexp_token_re/oamsxgc) {
    ...
    my %subparts = ( );
    foreach my $k (keys %+) { $subparts{$k} = $+{$k}; }
