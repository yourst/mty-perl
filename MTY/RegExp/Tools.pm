...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(rewind finished count_matches get_cap_group get_cap_groups
     condense_regexp optimize_regexp $regexp_chars_re get_entire_match
     get_match_length get_regexp_pattern matches_regexp_list
     get_regexp_modifiers get_pattern_if_regexp parse_non_overlapping
     valid_cap_group_count generate_prefix_regexp generate_suffix_regexp
     get_unmatched_remainder is_regexp_or_regexp_ref simple_regexp_optimizer
     is_regexp_simple_literal matches_regexp_list_index
     split_using_prefix_regexp split_using_suffix_regexp
     regexps_declared_in_package split_into_array_of_matches
     optimize_all_compiled_regexps get_regexp_capture_group_count
     adjust_default_regexp_modifiers generate_prefix_or_suffix_regexp
     get_regexp_pattern_and_modifiers split_lines_into_keys_and_values
     compile_regexp_list_of_alternatives
     get_capture_group_types_names_contents
     split_into_hash_of_key_sep_value_delim
     generate_regexp_to_match_any_string_in_list
     split_into_array_of_arrays_of_captured_groups
     split_into_array_of_hashes_of_captured_groups
...
sub generate_regexp_to_match_any_string_in_list {
  my $list = &array_args;
  if (!@$list) { return $always_fail_re; }
  my @sorted = sort_by_descending_length_then_alphabetical($list);
  my $re = join('|', map { quotemeta($_) } @sorted);
  return $re;
}
...
sub parse_non_overlapping($$) {
  ...
  my @list = ( );
  while ($input =~ /$regexp/oamsxgc) { push @list, $1; }
  return @list;
}
...
sub split_comma_delimited_list_with_parenthesized_sublists {
  my ($input) = @_;
  my @list = ( );
  while ($input =~ /$comma_delimited_list_item_with_parenthesized_sublists_re/oamsxg) 
    { push @list, $1; }
