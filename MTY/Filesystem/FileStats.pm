...
our @EXPORT = # (auto-generated by perl-mod-deps)
  qw(%file_type_to_description %stat_field_name_to_index_map
     @file_stat_field_names FILE_TYPE_ANONYMOUS_MOUNT FILE_TYPE_BIND_MOUNT
     FILE_TYPE_BLOCK_DEV FILE_TYPE_BLOCK_DEV_MOUNT FILE_TYPE_CHAR_DEV
     FILE_TYPE_DIR FILE_TYPE_FILE FILE_TYPE_FUSE_MOUNT FILE_TYPE_MOUNT_POINT
     ...
     PERM_FIELD_S PERM_FIELD_U PERM_R PERM_SETGID PERM_SETUID PERM_STICKY
     PERM_W PERM_X STAT_ACL STAT_ATIME STAT_ATIME_NS STAT_BLOCK_CHAR_DEV_SPEC
     STAT_BLOCK_COUNT STAT_BLOCK_SIZE STAT_CAPABILITIES STAT_CTIME
     STAT_CTIME_NS STAT_DEFAULT_ACL STAT_DEV STAT_EXTENTS STAT_FIELD_COUNT
     STAT_FSFLAGS STAT_FSTYPE STAT_GID STAT_INODE STAT_IS_ACCESSIBLE
     STAT_LOCKS STAT_MODE STAT_MTIME STAT_MTIME_NS STAT_NAME STAT_NLINK
     STAT_OPENFD STAT_PATH STAT_SIZE STAT_SYMLINK STAT_TYPE STAT_UID
     STAT_XATTRS S_IFMT_SHIFT dev_node_matches_major_minor_type
     file_stats_to_hash get_dev_node_major_minor_type get_file_type
     join_major_minor_dev mode_to_file_type split_major_minor_dev
     stat_path_or_fd);
...
sub stat_path_or_fd {
  # if we were passed a pre-existing stats structure:
  if (((ref $_[0]) // '') eq 'ARRAY') { return $_[0]; }

  my $no_follow_symlinks = (($_[1] // 0) != 0) ? AT_SYMLINK_NOFOLLOW : 0;

  my @info = (openhandle($_[0]))
    ? sys_fstatat(fileno($_[0]), '', AT_EMPTY_PATH | $no_follow_symlinks)
    : sys_fstatat(AT_FDCWD, $_[0], $no_follow_symlinks);
  return ((scalar @info) > 0) ? (wantarray ? (@info) : \@info) : undef;
}
...
our @file_stat_field_names = 
  qw(dev inode mode nlink uid gid dev_major_minor
     ...
     atime_ns ctime_ns mtime_ns path type symlink
     fstype fsflags xattrs acl default_acl 
     capabilities locks openfds extents
     accessible);
...
our %stat_field_name_to_index_map = (
  ...
  'nlink' => STAT_NLINK, 'n' => STAT_NLINK, 'links' => STAT_LINKS,
  ...
  'type' => STAT_TYPE, 't' => STAT_TYPE, # sort by predefined type-to-rank mapping
  ...
  'xattr' => STAT_XATTRS, 'xattrs' => STAT_XATTRS, 'extattrs' => STAT_XATTRS, 'ea' => STAT_XATTRS, # (sort by *number* of xattrs)
  'acl' => STAT_ACL, 'acls' => STAT_ACL, # sort by 0 = no acl (only POSIX u/g/o permissions), 1 = has ACL, 2 = has ACL and default ACL
  'defacl' => STAT_DEFAULT_ACL, 'defaultacl' => STAT_DEFAULT_ACL, # sort by 0 = no acl (only POSIX u/g/o permissions), 1 = has ACL, 2 = has ACL and default ACL
  'caps' => STAT_CAPABILITIES, 'capabilities' => STAT_CAPABILITIES, # sort by 0 = no acl (only POSIX u/g/o permissions), 1 = has ACL, 2 = has ACL and default ACL
  'locks' => STAT_LOCKS, 'lk' => STAT_LOCKS, # sort by number of locks on the file
  'openfd' => STAT_OPENFD, # sort by number of file handles to the file open by other processes
  'extents' => STAT_EXTENTS, 'cloned' => STAT_EXTENTS, # sort by number of bytes still shared with other files
  'accessible' => STAT_IS_ACCESSIBLE, 'ok' => STAT_IS_ACCESSIBLE, # accessible (r for files, x for directories) w.r.t. caller's effective permissions
);
...
#
# Test case (only tested on Linux 3.x for x86-64):
# 
#  $major = 0xAAAAAAAA;
#  $minor = 0xBBBBBBBB;
#
#  $joined = join_major_minor_dev($major, $minor);
#  ($split_major, $split_minor) = split_major_minor_dev($joined);
#  printf("joined = 0x%16lx\n", $joined);
#  printf("major  = 0x%16lx\n", $split_major);
#  printf("minor  = 0x%16lx\n", $split_minor);
#

sub dev_node_matches_major_minor_type($$$$) {
  my ($dev_node_path, $major, $minor, $type) = @_;
  # local (*dev_node_path, *major, *minor, *type) = \ (@_);
...
sub get_file_type {
  my $stats = stat_path_or_fd(@_);
  if (!defined $stats) { return undef; }
  return ($stats->[STAT_MODE] >> S_IFMT_SHIFT);
}

sub get_dev_node_major_minor_type {
  my $stats = stat_path_or_fd(@_) || return (wantarray ? ( ) : undef);
