...
# This is a packaged class module, so don't export any isolated functions:
...
x      if ($autoimport_label) {
    ...
    if (is_there($next_decl_label)) {
      if (($next_decl_label eq 'preserve') || ($next_decl_label eq 'nobundle')) {
        ...
        $m->add_module_to_bundle($1 // $default_bundle_name);
      }
    }
  ...
  if (!$skip_descent) {
    foreach $subnode (@$subnodes) {
      ...
      $m->analyze_ppi_subtree($subnode, $node, $level+1);
    }
  }
...
sub parse_module(+) {
  ...
  $m->analyze_ppi_subtree($tree, undef, 0);
...
sub generate_module_bundle(+$) {
  ...
  if (!sizeof(%bundle_name_to_list_of_modules)) {
    ...
    foreach $m (@$modules) {
      ...
      $m->add_module_to_bundle($default_bundle_name);
    }
  } else {
...
sub process_command_line {
  ...
  foreach $arg (@_) {
    ...
    if ($end_of_args || $arg =~ /^[^-]/oax) {
      ...
      my $m = MTY::PerlModDeps::Module->new($arg, sizeof(@mod...));
    } elsif ($arg eq '--') {
      $end_of_args = 1;
